...
class GoofishImClient:
    ...
    async def _solve_captcha_via_websocket(self, result: Dict[str, Any]) -> bool:
        ...
        if not (isinstance(resp, dict) and resp.get("success")):
            ...
            processing_conflict = bool(
                isinstance(resp_data, dict)
                and resp_data.get("processing_conflict")
            )

            # QR 登录后，websocket 服务通常已经在处理同一个账号的风控验证。
            # 旧逻辑收到“已有处理中的任务”后立即失败，导致在线聊天再次发起
            # 一套独立的滑块流程，既浪费验证次数，也会和自动回复争抢 Cookie/Token。
            # 这里等待 websocket 端把 Token 写入共享缓存；拿到后沿用同一个 Token，
            # 不再并发启动第二个滑块任务。
            if processing_conflict:
                logger.warning(
                    f"【{self.account_id}】检测到 websocket 端正在处理风控验证，"
                    "等待共享 Token 缓存，跳过重复滑块任务"
                )
                for wait_seconds in (2, 3, 5, 8, 12, 15, 15):
                    await asyncio.sleep(wait_seconds)
                    cached = await self._get_cached_token()
                    if cached:
                        self.device_id = cached["device_id"]
                        self._captcha_token_cache_saved = True
                        logger.info(
                            f"【{self.account_id}】已复用 websocket 端写入的 IM Token"
                        )
                        return True
                logger.warning(
                    f"【{self.account_id}】共享 Token 在等待窗口内仍未就绪"
                )
