...
class PlaywrightSliderService:
    ...
    def run(
        ...
        try:
            ...
            if "崩溃" in page_content or "STATUS_BREAKPOINT" in page_content:
                ...
                return False, None
            # 验证组件经常位于跨域 iframe；page.content() 只含主文档 HTML，不能据此
            # 判断“页面没有滑块”。直接复用元素查找器扫描所有 frame，避免主引擎把
            # 仍在 iframe 里的挑战误判成“无需验证”，随后返回空 cookie。
            page_has_captcha_hint = any(
                keyword in page_content.lower()
                for keyword in ["验证码", "captcha", "滑块", "slider"]
            )
            slider_button = slider_track = None
            try:
                _, slider_button, slider_track = self.element_finder.find_slider_elements()
            except Exception as find_error:
                logger.debug(f"【{self.pure_user_id}】预检滑块元素失败: {find_error}")
            has_slider = bool(slider_button and slider_track)

            if page_has_captcha_hint or has_slider:
                logger.info(
                    f"【{self.pure_user_id}】检测到滑块挑战"
                    f"（主文档提示={page_has_captcha_hint}, frame元素={has_slider}）"
                )
                ...
                # 没有元素时也不能把 punish 页当作已通过。只有离开挑战页并且没有
                # 滑块元素才表示本次无需处理。
                current_url = self.page.url or ""
                punish_kw = ("punish", "x5step=2", "action=captcha", "pureCaptcha", "/captcha")
                if any(keyword in current_url for keyword in punish_kw):
                    logger.warning(
                        f"【{self.pure_user_id}】仍在验证页但未找到可操作滑块，按失败处理: "
                        f"{current_url[:120]}"
                    )
                    return False, None
                logger.info(f"【{self.pure_user_id}】未检测到滑块挑战，本次无需验证")
                return True, None
    ...
    def _get_cookies_after_success(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            # 验证成功后页面通常会异步跳转；必须在等待后重新读取 URL。旧实现先
            # 读取再等待，容易把已经跳走的页面继续当作 punish 页，从而丢弃 x5sec。
            time.sleep(1)
