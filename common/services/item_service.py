...
class ItemService:
    ...
    async def list_items(self, owner_id: int | None, account_id: str | None = None) -> list[dict]:
        ...
        Args:
            ...
            account_id: 账号ID（可选）
        """
        # 商品必须关联现存账号。旧账号被删除后，历史目录记录可能成为
        # orphan row；使用 inner join 可避免这类记录以“空账号”出现在列表中。
        stmt = (
    ...
    async def list_items_paginated(
        ...
        # 总数查询必须使用同样的 inner join，否则列表隐藏孤儿记录而 total 仍把它们算进去，
        # 会出现总数与实际行数不一致、分页空页等问题。
        count_stmt = (
